# 단어사전(dictionary): Ridge Regression이용 만들기 
...
set.seed(12345)
Ridge_results <- cv.glmnet(X, Y, family = "binomial", alpha =0 , nfolds=4, type.mesure="class")

# plot(Ridge_results)
# plot(Ridge_results$glmnet.fit, xvar = "lambda")

coef_Ridge <- coef(Ridge_results, s = "lambda.min")[,1]

pos_Ridge <- coef_Ridge[coef_Ridge > 0]
neg_Ridge <- coef_Ridge[coef_Ridge < 0]

pos_Ridge <- sort(pos_Ridge, decreasing = T)
neg_Ridge <- sort(neg_Ridge, decreasing = F)


write.csv(pos_Ridge, "Ridge_posi_dic.csv")
write.csv(neg_Ridge, "Ridge_nega_dic.csv")
